...
class Node:
    ...
    def move_tile(action, data):
        moves = {"up": Node.move_up, "down": Node.move_down, "left": Node.move_left, "right": Node.move_right}
...
def solve_puzzle(start):
    ...
    while queue:
        ...
        for move in ["up", "down", "left", "right"]:
            new_state = Node.move_tile(move, node.data)
            if new_state is not None and tuple(map(tuple, new_state)) not in visited:
                queue.append(Node(0, new_state, node, move, 0))
...
def get_solution_path(node):
    path = []
    while node.parent:
        path.append((node.act, node.data))
        node = node.parent
    return path[::-1]


# Run the puzzle solver
initial_state = Node.get_initial()
